...
public class GoapPlanner
{
    public Queue<GoapAction> ActionPlan(List<GoapAction> actions, Dictionary<string, int> goal, WorldStates states)
    {
        ...
        Node start = new(null, 0, GoapWorld.Instance.GetWorld().GetStates(), states.GetStates(), null);
        ...
        while (n != null)
        {
            if (n.action != null)
            {
                result.Add(n.action); // Adiciona no final, o que é mais eficiente
            }
            n = n.parent;
        }
        Queue<GoapAction> queue = new Queue<GoapAction>(result.AsEnumerable().Reverse());

        // Para exibir as ações em Debug.Log
    ...
    bool BuildGraph(Node parent, List<Node> leaves, List<GoapAction> usableActions, Dictionary<string, int> goal)
    {
        ...
        foreach (GoapAction action in usableActions)
        {
            if (action.IsAchievableGiven(parent.state))
            {
                ...
                // Continua...
                if (GoalAchieved(goal, currentState)) // a construir
                {
                    leaves.Add(node);
                    foundPath = true;
                }
                else
                {
                    ...
                    List<GoapAction> subset = ActionSubset(usableActions, action); // a construir
